 ...
 * @Date: 2024-07-31 08:23:28
 * @LastEditors: albertxin albert_xin@qq.com
 * @LastEditTime: 2024-07-31 09:30:35
 * @FilePath: /shixinblog/blog/2024-07-31-记录terminal.md
...
```
### 统计未必对上的Reads，将未比对上的Reads转为fastq
```bash
samtools flagstat unmapped.bam > unmapped.stat
samtools fastq ../01.fq2Csa_rc/tophat_out/unmapped.bam >unmapped.fq
```    
### 利用tophat2将未比对上的Reads比对到Cfi_rc上
```bash
tophat2 -p 30 -N 1 --read-gap-length 1 --read-edit-dist 1 ../../re-seq/rc_genome/Cfi/bowtie2-index/Cfi_genome_rc ./unmapped.fq
samtools flagstat ./tophat_out/accepted_hits.bam > ./02.fq2Cfi_rec/accept_reads.stat
```
### 将比对结果转为fasta
```bash
samtools fasta  ./tophat_out/accepted_hits.bam >  ./03.mapped2Cfi_rc/mapped.fa
...
```
### 利用tblastx将Cfimapped的序列比对到Cficds上去
```bash
tblastx -query ../03.mapped2csacsarnaseq/mapped.fa -db ../../re-seq/rc_genome/Cfi/blastndb/Cfi_genome_rc_cds -out u02mappedfq2Cficds.tblastx.out -outfmt 6 -num_threads 40 -max_target_seqs 10  -evalue 1e-40